...
import { ArrowLeft, Wallet, Search, Check } from 'lucide-react';
...
const NewPaymentPage = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast({
      ...
      description: `Rs ${amountNum.toLocaleString('en-PK')} received from ${selectedCustomer?.name}`,
    });
  ...
  return (
    <BottomNav>
      <div className="page-container bg-background">
        {/* Header */}
        <header className="bg-success text-success-foreground px-4 py-4 sticky top-0 z-40">
          <div className="max-w-md mx-auto flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm opacity-80 font-urdu">رقم وصول کریں</p>
            </div>
          </div>
        </header>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    filteredCustomers.map((customer) => (
                      ...
                      >
                        ...
                        <div className="flex-1 text-left">
                          ...
                          <p className="text-xs opacity-70">Balance: Rs {customer.balance.toLocaleString('en-PK')}</p>
                        </div>
                        {selectedCustomerId === customer.id && (
            ...
            {selectedCustomer && (
              <div className="bg-secondary rounded-2xl p-4 animate-scale-in">
                <p className="text-sm text-muted-foreground mb-1">Receiving Payment From</p>
                <p className="font-bold text-lg">{selectedCustomer.name}</p>
                <p className="text-sm text-muted-foreground">
                  Pending Balance: <span className="text-warning font-semibold">Rs {selectedCustomer.balance.toLocaleString('en-PK')}</span>
                </p>
              </div>
            )}
            ...
            <div className="space-y-2 animate-slide-up" style={{ animationDelay: '50ms' }}>
              <Label htmlFor="amount" className="text-base font-semibold">
                💵 Payment Amount
                <span className="text-muted-foreground font-normal font-urdu text-sm ml-2">وصول رقم</span>
              </Label>
              <div className="relative">
              ...
              {selectedCustomer && selectedCustomer.balance > 0 && (
                <Button
                  ...
                  className="mt-2"
                >
                  Pay Full Balance: Rs {selectedCustomer.balance.toLocaleString('en-PK')}
                </Button>
              )}
            </div>
            ...
            <div className="space-y-2 animate-slide-up" style={{ animationDelay: '100ms' }}>
              <Label htmlFor="description" className="text-base font-semibold">
                📝 Note (Optional)
                <span className="text-muted-foreground font-normal font-urdu text-sm ml-2">نوٹ</span>
              </Label>
              <Textarea
                id="description"
                value={description}
            ...
            >
              ...
              Confirm Payment
              <span className="font-urdu ml-2">رقم وصول کریں</span>
            </Button>
          </form>
        </div>
      </div>
